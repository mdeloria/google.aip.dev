...
- Once an `Operation` has completed, meaning the `done` field is set to `true`,
  the response result **must be** immutable. More specifically, fetching a
  completed Operation **must always** return the same result
  up until the `Operation` reaches its terminal state (e.g. expiration).
